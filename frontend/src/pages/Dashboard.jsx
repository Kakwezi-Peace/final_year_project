...
/* ─────────────────────────────────────────────────
   CUSTOMER DASHBOARD
───────────────────────────────────────────────── */
const CustomerDashboard = ({ profile, bookings, payments, allBookings, allPayments, totalBookings, totalPayments, bPage, setBPage,
  ...
  const kpis = [
    ...
    { label: 'Pending / Active', value: pending + confirmed,    icon: Clock,         color: '#fbbf24', bg: 'rgba(251,191,36,0.1)',  border: 'rgba(251,191,36,0.25)' },
...
const AdminDashboard = ({ profile, bookings, payments, stats, allBookings, allPayments, bPage, setBPage,
  ...
  const kpis = [
    ...
    { label: 'Queue Length',    value: stats?.pendingBookings ?? '—', icon: Timer,  bg: '#3b82f6' },
    { label: 'Total Customers', value: (stats?.totalCustomers || 0).toLocaleString(), icon: Users, bg: '#f59e0b' },
  ];
  ...
  return (
    <div style={{ maxWidth: '1280px', margin: '0 auto', paddingBottom: '3rem' }}>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '1rem' }}>Here's what's happening at RUBIS Car Wash today</p>
      </div>

      {/* Peak Alert */}
      <div className="glass-panel" style={{
        border: '1px solid rgba(245,158,11,0.2)', borderLeft: '4px solid #f59e0b',
        borderRadius: '12px', padding: '1.25rem 1.5rem', display: 'flex', gap: '1rem',
        ...
        <div>
          <div style={{ fontWeight: '800', color: '#fbbf24', fontSize: '1rem' }}>Peak Hours Alert</div>
          <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: '2px' }}>
            High demand expected between 4:00 PM – 6:00 PM today.
          </div>
        </div>
      </div>

      {/* KPI Cards — no fake percentages */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.25rem', marginBottom: '2.5rem' }}>
        {kpis.map(k => (
          <div key={k.label} style={{
            background: k.bg, borderRadius: '16px', padding: '1.5rem',
            color: 'white', height: '120px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
          }}>
            <div style={{ fontSize: '0.72rem', fontWeight: '800', opacity: 0.9, textTransform: 'uppercase', letterSpacing: '0.07em' }}>{k.label}</div>
            <div style={{ fontSize: '2rem', fontWeight: '900' }}>{k.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(420px, 1fr))', gap: '2rem', marginBottom: '2.5rem' }}>
        <div className="glass-panel" style={{ padding: '2rem' }}>
          <h3 style={{ fontSize: '1rem', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '1.25rem' }}>Weekly Revenue</h3>
          <div style={{ height: '280px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={weeklyRevenueData}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--border-white)" />
                <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: 'var(--text-muted)' }} dy={10} />
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: 'var(--text-muted)' }} tickFormatter={v => `${v / 1000}k`} />
                <Tooltip contentStyle={{ background: 'var(--surface-solid)', border: '1px solid var(--border-white)', borderRadius: '8px', color: 'var(--text-primary)', fontSize: '12px' }} itemStyle={{ color: '#10b981' }} />
                <Line type="monotone" dataKey="revenue" stroke="#ef4444" strokeWidth={3} dot={{ r: 4, fill: '#ef4444', strokeWidth: 2, stroke: '#1a1a2e' }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        <div className="glass-panel" style={{ padding: '2rem' }}>
          <h3 style={{ fontSize: '1rem', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '1.25rem' }}>Cars Per Day</h3>
          <div style={{ height: '280px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={carsPerDayData}>
                ...
                <Tooltip cursor={{ fill: 'var(--surface)' }} contentStyle={{ background: 'var(--surface-solid)', border: '1px solid var(--border-white)', borderRadius: '8px', color: 'var(--text-primary)', fontSize: '12px' }} />
                <Bar dataKey="cars" fill="#3b82f6" radius={[4, 4, 0, 0]} barSize={36}>
      ...
      <div className="glass-panel" style={{ padding: '2rem', marginBottom: '2.5rem' }}>
          <h3 style={{ fontSize: '1.15rem', fontWeight: '800', marginBottom: '1.75rem' }}>7-Day Performance Breakdown</h3>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.06)', color: 'var(--text-muted)', fontSize: '0.8rem' }}>
                  {['Date', 'Day', 'Cars Processed', 'Revenue Generated', 'Categories Summation'].map(h => <th key={h} style={{ padding: '0.9rem 1rem', fontWeight: '700', textTransform: 'uppercase' }}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {carsPerDayData.map((d, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                    <td style={{ padding: '1rem', color: 'var(--text-muted)' }}>{d.desc}</td>
                    <td style={{ padding: '1rem', fontWeight: '800' }}>{d.day}</td>
                    <td style={{ padding: '1rem', color: '#60a5fa', fontWeight: '800' }}>{d.cars} <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 'normal' }}>cars</span></td>
                    <td style={{ padding: '1rem', color: '#10b981', fontWeight: '800' }}>{weeklyRevenueData[idx].revenue.toLocaleString()} <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 'normal' }}>RWF</span></td>
                    <td style={{ padding: '1rem', color: 'var(--text-secondary)', fontSize: '0.8rem' }}>{d.servicesStr}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async (bp = 0, pp = 0) => {
    try {
      ...
      const bEnd = isCust ? `/bookings/my-bookings?page=${bp}&size=5` : `/bookings?page=${bp}&size=5`;
      const pEnd = isCust ? `/payments/my-payments?page=${pp}&size=5` : `/payments?page=${pp}&size=5`;

      const [bRes, pRes, sRes, allBRes, allPRes] = await Promise.all([
        api.get(bEnd),
        api.get(pEnd),
        !isCust ? api.get('/reports/dashboard').catch(() => ({ data: null })) : Promise.resolve({ data: null }),
        isCust ? api.get(`/bookings/my-bookings?page=0&size=1000`) : api.get(`/bookings?page=0&size=1000`),
        isCust ? api.get(`/payments/my-payments?page=0&size=1000`) : api.get(`/payments?page=0&size=1000`)
      ]);
